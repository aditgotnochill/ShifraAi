(function () {
 ...
 <div class="shifra-overlay"></div>
  ...
  const loadAssistant = async () => {
    try {
      const res = await fetch(
        `https://shifraaiserver-fda3.onrender.com/api/assistant/config/${userId}`,
      );
  ...
  const speak = (text, onFinish) => {
    ...
    speech.onend = () => {
      status.innerText = "Tap button Speak";
  ...
  if (SpeechRecognition) {
    ...
    recognition.onresult = (e) => {
      ...
      setTimeout(async () => {
        try {
          ...
          const res = await fetch("https://shifraaiserver-fda3.onrender.com/api/assistant/ask", {
            method: "POST",
            headers: {
    ...
    recognition.onerror = () => {
      status.innerText = "Tap buttton to Speak";
      wave.style.opacity = "0";
    };
  } else {
